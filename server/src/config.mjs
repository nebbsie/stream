...
const DOMAIN = String(env.CATHODE_DOMAIN ?? '').trim().replace(/^https?:\/\//, '').replace(/\/+$/, '')

const TURN_URLS_SET = list(env.CATHODE_TURN_URLS)
...
export const PEER_HEADERS = { authorization: `Bearer ${CLUSTER_SECRET}`, 'x-cathode-peer': PUBLIC_URL }
