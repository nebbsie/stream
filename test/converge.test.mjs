...
// Runs in the page, because the log is a browser module that wants real crypto.
const HARNESS = `
  const { RoomLog, makeEvent } = await import('/src/store/log.ts')

  // Seeded, so a failure can be looked at again.
  const rng = (seed) => () => {
  ...
  // What a device would draw, not the event set: the claim is that two logs say the same thing.
  const shape = (log) => {
    const parts = []
...
try {
  ...
  const made = await page.evaluate(
    new Function(`return (async () => {
        ${HARNESS}
        window.__h = { RoomLog, makeEvent, shuffled, shape, build, KEYS }

        // Signed for real: makeEvent stamps the id from the content, and the id is half of the sort order.
        let clock = 1000
        const events = []
        const push = async (author, kind, body) => {
          clock += 1
          const e = await makeEvent('room', author, clock, kind, body)
          events.push(e)
          return e
        }
        ...
        await push(CAROL, 'said', { text: 'in plans', channel: 'plans' })
