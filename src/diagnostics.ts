 ...
 *
 * Beam checks this before the host presses Start, so nobody meets a broken
...
export function hostBlocker(s: Support): string | null {
  if (!s.secureContext) {
    return 'Beam needs HTTPS. Open the site over https, or use http://localhost while you develop.'
  }
  if (!s.webrtc) {
    return `${s.browser} has no WebRTC, so it cannot run Beam.`
  }
  if (s.isIOS) {
...
export function viewerBlocker(s: Support): string | null {
  if (!s.secureContext) return 'Beam needs HTTPS to watch a stream.'
  if (!s.webrtc) return `${s.browser} has no WebRTC, so it cannot watch a Beam stream.`
  return null
}
...
export function hostNotes(s: Support): string[] {
  ...
  if (!hasTurn()) {
    notes.push(
      'Beam runs with no relay server. About one viewer in eight on a strict network will fail to connect.',
    )
  }
  return notes
}
